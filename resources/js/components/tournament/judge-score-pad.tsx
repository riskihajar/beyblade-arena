...
export function JudgeScorePad({ match }: Props) {
    const [selectedWinnerId, setSelectedWinnerId] = useState<string | null>(match.player1_id || null);
    ...
    const [walkoverPresentId, setWalkoverPresentId] = useState<string>(match.player1_id || '');

    const isCompleted = match.status === 'completed' || match.status === 'walkover';
    const isDisputed = match.status === 'disputed' || match.is_disputed;

    const targetPoints = Number(match.ruleset_snapshot?.points_to_win ?? match.category?.target_points ?? 4);
    ...
    const player1Name = match.player1?.display_nickname || match.player1?.user?.name || 'Blader 1';
    const player2Name = match.player2?.display_nickname || match.player2?.user?.name || 'Blader 2';
    ...
    return (
        <div className="space-y-6 max-w-xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
                {/* Player 1 Card */}
                <div
                    onClick={() => !isCompleted && setSelectedWinnerId(match.player1_id || null)}
                ...
                >
                    <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground mb-1">
                        ...
                        <span className="font-semibold truncate">{player1Name}</span>
                    </div>
                    <div className="text-5xl font-black font-mono tracking-tight text-foreground py-2">
                        {match.player1_score}
                    </div>
                    <div className="text-[11px] font-medium text-primary mt-1">
                        {selectedWinnerId === match.player1_id ? 'Pemenang Ronde Aktif' : 'Klik untuk Pilih'}
                    </div>
                </div>

                {/* Player 2 Card */}
                <div
                    onClick={() => !isCompleted && setSelectedWinnerId(match.player2_id || null)}
                    className={`cursor-pointer rounded-xl border-2 p-4 text-center transition-all ${
                ...
                >
                    <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground mb-1">
                        <User className="size-3.5" />
                        <span className="font-semibold truncate">{player2Name}</span>
                    </div>
                    <div className="text-5xl font-black font-mono tracking-tight text-foreground py-2">
                        {match.player2_score}
                    </div>
                    <div className="text-[11px] font-medium text-primary mt-1">
                        {selectedWinnerId === match.player2_id ? 'Pemenang Ronde Aktif' : 'Klik untuk Pilih'}
                    </div>
                </div>
            </div>
            ...
            {isCompleted && (
                <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-center space-y-1">
                    <div className="inline-flex size-10 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-600 mb-1">
                    ...
                    <p className="text-xs text-emerald-800 dark:text-emerald-300">
                        Pemenang: <span className="font-bold">{match.winner?.display_nickname || match.winner?.user?.name}</span> ({match.player1_score} - {match.player2_score})
                    </p>
                </div>
            )}
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
                        ...
                        >
                            ...
                            <span className="text-[10px] text-muted-foreground font-normal">+1 Poin</span>
                        </Button>
                        ...
                        >
                            ...
                            <span className="text-[10px] text-muted-foreground font-normal">+2 Poin</span>
                        </Button>
                        ...
                        <Button
                            type="button"
                            size="lg"
                            className="h-14 flex-col gap-0.5 bg-linear-to-r from-red-600 to-amber-600 hover:from-red-700 hover:to-amber-700 text-white font-black text-xs shadow-xs"
                        ...
                        >
                            ...
                            <span className="text-[10px] font-normal opacity-90">+3 Poin</span>
                        </Button>
                    </div>
            ...
            {match.battles && match.battles.length > 0 && (
                <div className="rounded-xl border bg-card p-4 space-y-3">
                    <span className="text-xs font-bold text-foreground flex items-center gap-1.5">
                        ...
                        Riwayat Ronde Battle (Total {match.battles.length} Ronde)
                    </span>
                    <div className="space-y-1.5">
                        {match.battles.map((b) => (
                            <div
                                key={b.id}
                                className="rounded-md border bg-muted/30 px-3 py-2 text-xs flex items-center justify-between"
                            >
                                <div className="flex items-center gap-2">
                                    <span className="font-bold text-primary">Ronde #{b.battle_number}</span>
                                    {b.is_draw ? (
                                        ...
                                        <span>
                                            <span className="font-semibold">{b.winner?.display_nickname || b.winner?.user?.name}</span>{' '}
                                            <span className="text-muted-foreground uppercase text-[10px]">({b.finish_type})</span>
                                        </span>
                                    )}
                                </div>
                                <span className="font-mono font-semibold text-foreground">
                                    {b.player1_points_after} - {b.player2_points_after}
                                </span>
                            </div>
                        ))}
                    </div>
            ...
            {!isCompleted && (
                <div className="pt-2 text-center">
                    <button
                        ...
                        className="text-xs text-muted-foreground hover:text-destructive underline inline-flex items-center gap-1"
                    >
            ...
            {/* Walkover Dialog */}
            <Dialog open={walkoverModalOpen} onOpenChange={setWalkoverModalOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tetapkan Walkover jika salah satu peserta tidak hadir di arena setelah batas panggilan 3 menit habis.
                        </DialogDescription>
                    </DialogHeader>

                    <div className="space-y-4 py-2 text-xs">
                        <Field name="walkover_player">
                            <FieldLabel>Pemenang WO (Peserta yang Hadir)</FieldLabel>
                            <select
                                value={walkoverPresentId}
                                onChange={(e) => setWalkoverPresentId(e.target.value)}
                                className="h-9 w-full rounded-md border border-input bg-background px-3 text-xs"
                            >
                                <option value={match.player1_id || ''}>{player1Name}</option>
                                <option value={match.player2_id || ''}>{player2Name}</option>
                            </select>
                        </Field>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setWalkoverModalOpen(false)}>
                            Batal
                        </Button>
                ...
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <div className="flex items-center gap-2 text-destructive">
                            ...
                            <DialogTitle>Laporkan Sengketa / Dispute</DialogTitle>
                        </div>
                        <DialogDescription>
                            Kunci sementara pertandingan ini untuk peninjauan langsung oleh Head Judge / Panitia.
                        </DialogDescription>
                    </DialogHeader>
