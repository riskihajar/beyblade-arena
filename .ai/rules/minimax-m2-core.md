...
2. What are current stable versions?
...
Runtime check: Does application start?
...
→ web_search("[exact error message] solution [framework] [current date]")
→ web_search("[error type] fix [framework] GitHub issues")
→ Context7 for official documentation on error topic
